...
def check_def_flows(target_str='', mode='count_flow'):
        ...
        if True: #"record_" in fpath:
            ...
            with codecs.open(fpath, mode='r') as ff:
                ...
                for num, line in enumerate(cont, 1):
                    if 'type = inactive' in line or 'type = forin' in line or 'type = for_in' in line:
                        inactive_flag = True
                        continue
    ...
    with open('/media/datak/inactive/analysis/forin/' + date.today().strftime("%m_%d_") + TARGET_CRAWL + '_consequence.log', 'w') as f1: #0614_consequence_cookie.log, 0609_consequence_0to600kplus.log
...
if __name__ == "__main__":
    check_def_flows(target_str='oracle_forin_test_', mode='count_flow')
