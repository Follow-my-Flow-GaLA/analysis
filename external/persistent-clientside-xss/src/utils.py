...
from urllib.parse import urlsplit, urlunsplit, parse_qs, parse_qsl
...
def recursive_replace(data_in, replace_value, replace_with):
    ...
    if isinstance(data_in, dict):
        ...
        for element in data_in:
            if isinstance(element, bytes) or isinstance(element, str):
...
def find_match(items, tainted_value):
    ...
    """
    tainted_value = tainted_value.encode('ascii').decode("ascii", "ignore")
    matches = []
