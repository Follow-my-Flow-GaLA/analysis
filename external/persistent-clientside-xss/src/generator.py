...
import json
from pyesprima.pyesprima import parse as prs
from pyesprima.pyesprima import tokenize as tkz
...
def get_js_exploit(finding):
    ...
    try:
        parsed_value = prs(finding["value"], range=True)
    ...
    for source in finding["sources"][:20]:
        ...
        try:
            ...
            parsed_exploit = prs(code, range=True)
        except RuntimeError as e:
...
def get_html_exploit(finding):
    ...
    payload = f"<img src=1 onerror={validation_payload}>"
                        ...
                        if url_so_far.count("/") < 3:
                            ...
                            if parsed.netloc in source["value_part"] or source["value_part"] in parsed.netloc:
                                ...
                                if exploit_url:
                                    exploits.append(createWebExploit(exploit_url, source["id"]))
                                    continue
            # We have a generated a breakout sequence and can make use of it now
