...
# usage: sudo bash oracle-phase4-auto-recursive.sh 173680 300000 2 16 0 2>&1 | grep -vE "Xlib:  extension \"RANDR\" missing on display \".*\"\.|Xlib:  extension \"XInputExtension\" missing on display \".*\"\.|ERROR:zygote_communication_linux.cc\(292\)\] Failed to" | tee -a ./oracle_phase4_1M_looping.log
...
SITE_TO_CRAWL="50-500k-to-crawl-phase2.csv" # "1M-to-crawl-phase4.csv"
...
do
    ...
    then
            ...
            out/Oracle-release/chrome ${url} --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --inactive_taint_enable --current_site=${url} --undef_prop_dataset_path=\"http://localhost:5000/api/phase3/new_undef\"" \
