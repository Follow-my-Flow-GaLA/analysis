...
#usage: sudo bash detector-phase3-auto-recursive.sh 386560 1000000 2 20 0 2>&1 | grep -vE "Xlib:  extension \"RANDR\" missing on display \".*\"\.|Xlib:  extension \"XInputExtension\" missing on display \".*\"\.|ERROR:zygote_communication_linux.cc\(292\)\] Failed to" | tee -a ./detector_phase3_db_crawling.log
...
do
    ...
    then
            ...
            out/Oracle-release/chrome ${url} --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition --phase3_enable --current_site=${url}" \
                     --user-data-dir=/tmp/${NAME} --load-extension=/home/zfk/Documents/crawler-extension,/home/zfk/Documents/inject_pp_extension/ --new-window --no-sandbox --disable-gpu --disable-hang-monitor --ignore-certificate-errors --allow-insecure-localhost &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < <(grep . /media/datak/inactive/analysis/phase3/count_websites/phase3_data_to_change_non_dummy_website_list.txt ) #tranco_253N9-1m.csv) #websites_total_to_pp_pattern1_600kto1m.txt) #42_websites_url_src_to_pp_again_0to600kplus.txt)  #websites_total_to_pp_pattern1_0to600kplus.txt #websites_to_pp_pattern1_0to200k.txt
