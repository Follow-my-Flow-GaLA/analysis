...
# usage: sudo bash detector-phase1-auto-recursive.sh 996050 1000000 2 25 1 2>&1 | grep -vE "Xlib:  extension \"RANDR\" missing on display \".*\"\.|Xlib:  extension \"XInputExtension\" missing on display \".*\"\.|ERROR:zygote_communication_linux.cc\(292\)\] Failed to" | tee -a ./detector_phase1_1M_crawling.log
...
do
    ...
    then
            ...
            out/Detector-release/chrome ${url} --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition --inactive_conseq_log_enable --current_site=${url}" \
...
done < <(grep . ${SAVE_PATH}/src/tranco_LJ494.csv ) #tranco_253N9-1m.csv) #websites_total_to_pp_pattern1_600kto1m.txt) #42_websites_url_src_to_pp_again_0to600kplus.txt)  #websites_total_to_pp_pattern1_0to600kplus.txt #websites_to_pp_pattern1_0to200k.txt
