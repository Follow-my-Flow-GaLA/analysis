...
#usage: sudo bash show-load-time-gala-1k.sh 0 1000 2 15
...
do
    ...
    then
	    ...
	    out/Oracle-release/chrome ${url}/?KEY1[KEY2]=VALUE\&KEY1.KEY2=VALUE\&KEY1[KEY2][KEY3]=VALUE --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --inactive_taint_enable --current_site=${url} --no-crankshaft --no-turbo" --user-data-dir=/tmp/${NAME} --load-extension=/home/zfk/Documents/process-cookies/taintchrome/show_load_time_extension --new-window --no-sandbox --disable-gpu --disable-hang-monitor --enable-logging=stderr &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file &
