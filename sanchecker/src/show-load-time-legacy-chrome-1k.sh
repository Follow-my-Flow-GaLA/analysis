...
#usage: sudo bash show-load-time-ndss18-1k.sh 0 1000 2 20
...
cd /home/zfk/Documents/chrome-linux/
...
do
    ...
    then
            ...
            ./chrome ${url}/?KEY1[KEY2]=VALUE\&KEY1.KEY2=VALUE\&KEY1[KEY2][KEY3]=VALUE \
                     --user-data-dir=/tmp/${NAME} --load-extension=/home/zfk/Documents/process-cookies/taintchrome/show_load_time_extension --new-window --disable-gpu --no-sandbox --disable-hang-monitor --enable-logging=stderr &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < <(grep . ${SAVE_PATH}/src/tranco_LJ494.csv) 
