...
#usage: sudo bash show-load-time-key1key2-1k.sh 0 1000 2 15
...
then
        ...
        TAG="show_load_time_Fastchrome_key1key2_1k_"
        if ((if_flush == 0))
        then
...
do
    ...
    then
            ...
            out/Fast/chrome ${url}/?KEY1[KEY2]=VALUE\&KEY1.KEY2=VALUE\&KEY1[KEY2][KEY3]=VALUE --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
