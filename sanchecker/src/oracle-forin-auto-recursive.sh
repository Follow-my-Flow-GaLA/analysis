...
#usage: sudo bash oracle-forin-auto-recursive.sh 0 10000 2 20 0 2>&1 | tee -a ./oracle_forin_crawling.log
...
do
    ...
    then
            ...
            out/Oracle-debug/chrome ${url} --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --inactive_taint_enable --should_taint_forin --no-crankshaft --no-turbo" \
...
done < <(grep . /home/zfk/Documents/sanchecker/src/tranco_253N9-1m.csv) #websites_total_to_pp_pattern1_600kto1m.txt) #42_websites_url_src_to_pp_again_0to600kplus.txt)  #websites_total_to_pp_pattern1_0to600kplus.txt #websites_to_pp_pattern1_0to200k.txt
